...
namespace FakeXrmEasy.Core.Tests.Query.FetchXml.JoinOperatorTests
{
    public class InOperatorTests: FakeXrmEasyTestsBase
    {
        ...
        [Fact]
        public void Should_translate_link_entity_with_in_operator_that_matches_an_account_record()
        {
            ...
            var fetchXml = @"
                    <fetch distinct='false' useraworderby='false' no-lock='false' mapping='logical'>
                        <entity name='contact'>
                            ...
                            <link-entity name='account' to='contactid' from='primarycontactid' link-type='in'>
                                <filter type='and'>
            ...
            Assert.Equal(JoinOperator.In, linkEntity.JoinOperator);
